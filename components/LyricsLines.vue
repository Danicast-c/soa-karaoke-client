  ...
  <v-container>
    <v-data-table striped hover dark :items="lines" :headers="fields">
      <template v-slot:item.text="{ value, item }">
        ...
        <v-text-field
          v-else
          v-model="edit_line.text"
          :rules="[() => !!edit_line.text || 'This field is required']"
        />
      </template>
      ...
      <template v-slot:item.end_s="{ value, item }">
        ...
        <v-text-field
          v-else
          v-model="edit_line.end_s"
          :rules="[() => validateMmSsMs(edit_line.end_s)]"
        />
      </template>
...
export default {
  ...
  methods: {
    onEdit(index) {
      if (
        this.edit == index &&
        this.validateMmSsMs(this.edit_line.start_s) &&
